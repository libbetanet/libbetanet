...
int htx_init(htx_ctx_t *ctx, const uint8_t c2s_key[HTX_KEY_LEN],
             const uint8_t c2s_salt[HTX_NONCE_LEN],
             const uint8_t s2c_key[HTX_KEY_LEN],
             const uint8_t s2c_salt[HTX_NONCE_LEN]) {
  if (!ctx)
    return -1;
...
size_t htx_pack_stream(htx_ctx_t *ctx, htx_dir_t dir, uint64_t stream_id,
  ...
  return htx_pack_frame(out, out_cap, HTX_T_STREAM, pkt, true, stream_id, pt,
                        pt_len, dk->key, dk->salt);
}
...
size_t htx_pack_close(htx_ctx_t *ctx, htx_dir_t dir, uint64_t close_code,
                      ...
                      size_t out_cap) {
  if (reason_len > HTX_CLOSE_REASON_MAX) {
    return HTX_ERR_LENCAP;
  }
  htx_dir_keys_t *dk = (dir == HTX_DIR_C2S) ? &ctx->c2s : &ctx->s2c;
  uint64_t pkt = dk->next_pkt_num++;

  uint8_t pt[HTX_CLOSE_REASON_MAX + 16];
...
int htx_unpack_any(htx_ctx_t *ctx, htx_dir_t dir, const uint8_t *buf,
                   size_t len, htx_hdr_info *info_out, uint8_t *pt_out,
                   size_t pt_cap) {
  const htx_dir_keys_t *dk = (dir == HTX_DIR_C2S) ? &ctx->c2s : &ctx->s2c;
  return htx_unpack_frame(buf, len, dk->key, dk->salt, info_out, pt_out,
                          pt_cap);
}
